package com.bauerfinanz.gutachten.common.service;
...
import com.bauerfinanz.gutachten.common.to.SimulationResult;
import com.bauerfinanz.gutachten.common.to.SparplanParameter;

import jakarta.enterprise.context.ApplicationScoped;
import lombok.RequiredArgsConstructor;

/**
 * Gutachten data business logic. w
 * 
 * @author Stefan Wiessner
...
public class SparplanService {
	private SparplanParameter params;
	private Deque<Map<String, Object>> portfolio;
	private List<Map<String, Object>> rebalancingLog;
	private List<Map<String, Object>> monatlicheKostenLogs;
	...
	private double[] abschlusskostenMonatlichRest;
	private double[] abschlusskostenEinmaligRest;

	private void initializeSimulation(final SparplanParameter params) {
		this.params = params;
		...
		params.fullTaxRate = params.abgeltungssteuerRate
				* (1 + params.soliZuschlagOnAbgeltungssteuer + params.kirchensteuerOnAbgeltungssteuer);
	...
	public SimulationResult runSimulation(final SparplanParameter params) {
		initializeSimulation(params);
		for (int month = 0; month < params.laufzeit * 12; month++) {
			simulateMonth(month);
		}
		finalizeSimulation();
		return SimulationResult.builder().kostenLogs(monatlicheKostenLogs).rebalancingLogs(rebalancingLog)
				.cashflows(cashflows).build();
	}
	...
	private void handleMonthlyInvestment(int month, LocalDate currentDate) {
		...
		boolean isRegelmaessig = params.regelSonderzahlungTurnusJahre > 0 && month > 0
				&& month % (params.regelSonderzahlungTurnusJahre * 12) == 0;
	...
	private void handleTaxes(LocalDate currentDate) {
		...
		if (!params.versicherungModus && isJanuary) {
			for (Map<String, Object> entry : portfolio) {
				...
				double steuerfreibetrag = Math.min(freistellungsTopf,
						steuerbarerErtrag * (1 - params.teilfreistellung));
				...
				if (steuer > 0) {
					...
					entry.put("vorabpauschalenBereitsVersteuert",
							(double) entry.getOrDefault("vorabpauschalenBereitsVersteuert", 0.0) + zuVersteuern);
					totalTaxPaid += steuer;
		...
		if (!params.versicherungModus && currentDate.getMonthValue() == 12 && params.rebalancingRate > 0) {
			...
			if (umzuschichten > 0) {
				...
				while (remaining > 1e-9 && !portfolio.isEmpty()) {
					...
					double vorabUsed = Math.min(
							(double) entry.getOrDefault("vorabpauschalenBereitsVersteuert", 0.0) * prop,
							steuerbarerGewinn);
					...
					entry.put("vorabpauschalenBereitsVersteuert", Math.max(0.0,
							(double) entry.getOrDefault("vorabpauschalenBereitsVersteuert", 0.0) - vorabUsed));
	...
	private void finalizeSimulation() {
		...
		if (restwert > 1e-9) {
			...
			if (params.versicherungModus) {
				...
				steuer = gewinn * (aktuelleAlter >= 62 && aktuelleLaufzeit >= 12 ? 0.5 : 0.85)
						* params.persoenlicherSteuersatz;
			} else {
				double steuerbar = gewinn * (1 - params.teilfreistellung);
				double bereitsVersteuert = portfolio.stream()
						.mapToDouble(e -> (double) e.getOrDefault("vorabpauschalenBereitsVersteuert", 0.0)).sum();
